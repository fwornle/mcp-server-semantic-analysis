...
export class ObservationGenerationAgent {
  ...
  private async generateFromSemanticAnalysis(semanticAnalysis: any): Promise<StructuredObservation[]> {
    ...
    if (semanticAnalysis.entities && Array.isArray(semanticAnalysis.entities)) {
      log('Processing semantic entities from batch with LLM synthesis', 'info', {
  ...
  /**
   * Create observation from a batch entity with LLM synthesis
   * Synthesizes richer, actionable observations from raw entity data
   */
  private async createEntityObservation(entity: any): Promise<StructuredObservation | null> {
    ...
    const rawContent = Array.isArray(entityObservations)
...
Provide a JSON response with:
{
  ...
  "confidence": number // 0-1
    ...
    const observationContent = synthesizedObservation?.synthesizedContent
      || (rawContent.length > 50 ? rawContent : `Entity: ${entity.name}`);

    const guidance = synthesizedObservation?.actionableGuidance || '';
    const fullContent = guidance
      ? `${observationContent}\n\nGuidance: ${guidance}`
      : observationContent;
    ...
    return {
      ...
      metadata: {
        ...
        processingPhase: 'batch-analysis',
        tags: [...(entity.tags || []), ...(synthesizedObservation?.relatedConcepts || [])],
        llmSynthesized: !!synthesizedObservation,
        keyPattern: synthesizedObservation?.keyPattern || null
      },
